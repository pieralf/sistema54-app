...
export default function EditInterventionPage() {
      ...
      try {
        ...
        if (intervento.cliente_id) {
          ...
          if (cliente.has_noleggio && cliente.assets_noleggio) {
            const assetsPrinting = cliente.assets_noleggio.filter((a: any) => a.tipo_asset === "Printing");
            ...
            for (const asset of assetsPrinting) {
              try {
                const lettureRes = await axios.get(`${API_URL}/letture-copie/asset/${asset.id}/ultima`, {
  ...
  const handleSignatureConfirm = async () => {
    ...
    // Validazione prelievo copie: se è attivo, tutte le macchine devono avere le copie inserite
    ...
    if (formData.is_prelievo_copie && macroCategoria === "Printing & Office") {
      ...
      const macchineSenzaLetture = assetsPrinting.filter((asset: any) => {
        const lettura = lettureCopie[asset.id];
        return !lettura || !lettura.contatore_bn || (asset.is_colore && !lettura.contatore_colore);
      });
      
      if (macchineSenzaLetture.length > 0) {
        setIsSubmitting(false);
        alert(`Devi inserire le letture copie per tutte le macchine a noleggio:\n${macchineSenzaLetture.map((a: any) => `${a.marca} ${a.modello}`).join('\n')}`);
        return;
      }
    }
      ...
      if (formDataTemp.is_prelievo_copie && macroCategoria === "Printing & Office") {
        ...
        for (const asset of assetsPrinting) {
          ...
          if (lettura && lettura.contatore_bn) {
            try {
              await axios.post(`${API_URL}/letture-copie/`, {
                asset_id: asset.id,
