...
export default function NuovoClientePage() {
  ...
  const loadCliente = async () => {
    try {
      const res = await axios.get(`${API_URL}/clienti/${id}`);
  ...
  const [newAsset, setNewAsset] = useState<any>({
    ...
    contatore_iniziale_bn: 0,
    contatore_iniziale_colore: 0,
  ...
  const onSubmit = async (data: any) => {
    ...
    try {
      // Prepara gli assets rimuovendo i campi temporanei tipo_configurazione
      ...
      if (isEdit) {
        await axios.put(`${API_URL}/clienti/${id}`, payload);
        alert('Cliente aggiornato con successo!');
      } else {
        await axios.post(`${API_URL}/clienti/`, payload);
                    ...
                    <>
                      ...
                      {watch('has_multisede') && sedi.length > 0 && (
                        <div>
                          ...
                          <select
                            ...
                            onChange={(e) => setNewAsset({ ...newAsset, sede_id: e.target.value ? parseInt(e.target.value) : null })}
                            className="w-full bg-white border border-gray-200 rounded-xl py-3 pl-4 pr-4 text-sm focus:ring-2 focus:ring-blue-500 shadow-sm outline-none"
                          >
                      ...
                      <div className="grid grid-cols-2 gap-3">
                        <IOSInput
                          ...
                          type="number"
                          value={newAsset.contatore_iniziale_bn || 0}
                          onChange={(e) => setNewAsset({ ...newAsset, contatore_iniziale_bn: parseInt(e.target.value) || 0 })}
                        />
                        <IOSInput
                          label="Contatore Iniziale Colore"
                          type="number"
                          value={newAsset.contatore_iniziale_colore || 0}
                          onChange={(e) => setNewAsset({ ...newAsset, contatore_iniziale_colore: parseInt(e.target.value) || 0 })}
