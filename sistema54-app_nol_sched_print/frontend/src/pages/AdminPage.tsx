...
export default function AdminPage() {
  ...
  const loadAllCounts = async () => {
    ...
    try {
      ...
      const [usersRes, clientiRes, interventiRes, magazzinoRes] = await Promise.allSettled([
        ...
        axios.get(`${API_URL}/clienti/?q=`),
        axios.get(`${API_URL}/interventi/`),
        axios.get(`${API_URL}/magazzino/?q=`)
      ]);
  ...
  const loadData = async () => {
    ...
    try {
      if (activeTab === 'users') {
        ...
        console.log('Caricamento clienti...', { API_URL, searchTerm, authHeader: axios.defaults.headers.common['Authorization'] });
        ...
        const res = await axios.get(`${API_URL}/interventi/`);
  ...
  const handleCreateProdotto = async (data: any) => {
    try {
      await axios.post(`${API_URL}/magazzino/`, data);
  ...
  const handleOpenPDF = async (interventoId: number) => {
    try {
      ...
      try {
        const interventoResponse = await axios.get(`${API_URL}/interventi/${interventoId}`);
      ...
      const response = await axios.get(`${API_URL}/interventi/${interventoId}/pdf`, {
        responseType: 'blob',
      });
  ...
  return (
    <div className="min-h-screen bg-gray-50 pb-20 font-sans">
      ...
      <main className="max-w-6xl mx-auto p-4 sm:p-6">
        ...
        {/* Search Bar per Clienti e Magazzino */}
        {(activeTab === 'clienti' || activeTab === 'magazzino') && (
          <div className="mb-6">
            ...
            {activeTab === 'users' && (
              <IOSCard>
                <div className="flex justify-between items-center mb-4">
                  ...
                  <button className="flex items-center gap-2 px-4 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 text-sm font-semibold">
