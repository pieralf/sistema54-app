...
export default function TwoFactorSettings() {
  ...
  const handleSetup = async () => {
    ...
    setSuccess('');
    try {
      const res = await axios.post(`${API_URL}/api/auth/2fa/setup`);
  ...
  const handleEnable = async () => {
    ...
    setSuccess('');
    try {
      await axios.post(`${API_URL}/api/auth/2fa/enable`, { code: verificationCode });
      setIsEnabled(true);
      setQrCode('');
      setVerificationCode('');
  ...
  const handleDisable = async () => {
    ...
    setSuccess('');
    try {
      await axios.post(`${API_URL}/api/auth/2fa/disable`, { code: verificationCode });
      setIsEnabled(false);
      setQrCode('');
      setBackupCodes([]);
  ...
  const handleRegenerateBackup = async () => {
    ...
    setSuccess('');
    try {
      const res = await axios.post(`${API_URL}/api/auth/2fa/regenerate-backup`);
      setBackupCodes(res.data.backup_codes);
      setShowBackupCodes(true);
